...
export default function UserManagement() {
  ...
  const [deleteActivityTarget, setDeleteActivityTarget] = useState<ActivityLog | null>(null);
  const [clearActivitiesConfirm, setClearActivitiesConfirm] = useState(false);
  ...
  const deleteCustomer = useMutation({
    mutationFn: async ({ userId, email }: { userId: string; email: string | null }) => {
      if (email && email.toLowerCase() === "bdinfosky@gmail.com") {
        throw new Error("সুপার অ্যাডমিন অ্যাকাউন্ট ডিলিট করা সম্ভব নয়।");
      }
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                {activities && activities.length > 0 && (
                  <Button
                    ...
                    onClick={() => setClearActivitiesConfirm(true)}
                  >
      ...
      <AlertDialog open={!!deleteCustomerTarget} onOpenChange={(v) => !v && setDeleteCustomerTarget(null)}>
        <AlertDialogContent className="rounded-2xl border-border/50">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display font-bold">গ্রাহক অ্যাকাউন্ট ডিলিট নিশ্চিতকরণ</AlertDialogTitle>
            <AlertDialogDescription>
              আপনি কি নিশ্চিতভাবে <strong>"{deleteCustomerTarget?.name}"</strong> এর অ্যাকাউন্টটি চিরতরে ডিলিট করতে চান? এর ফলে তার লগইন অ্যাকাউন্ট এবং প্রোফাইল সম্পূর্ণ মুছে যাবে এবং এটি পুনরুদ্ধার করা সম্ভব হবে না।
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2">
            ...
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90 rounded-xl" onClick={() => {
              if (deleteCustomerTarget) {
                deleteCustomer.mutate({ userId: deleteCustomerTarget.user_id, email: deleteCustomerTarget.email });
      ...
      <AlertDialog open={!!deleteActivityTarget} onOpenChange={(v) => !v && setDeleteActivityTarget(null)}>
        <AlertDialogContent className="rounded-2xl border-border/50">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display font-bold">অ্যাক্টিভিটি লগ ডিলিট নিশ্চিতকরণ</AlertDialogTitle>
            <AlertDialogDescription>
              আপনি কি নিশ্চিতভাবে এই অ্যাক্টিভিটি লগটি মুছে ফেলতে চান?
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="rounded-xl">বাতিল</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90 rounded-xl" onClick={() => {
              if (deleteActivityTarget) {
                deleteActivity.mutate(deleteActivityTarget.id);
      ...
      <AlertDialog open={clearActivitiesConfirm} onOpenChange={setClearActivitiesConfirm}>
        <AlertDialogContent className="rounded-2xl border-border/50">
          <AlertDialogHeader>
            <AlertDialogTitle className="font-display font-bold">সব অ্যাক্টিভিটি লগ মুছুন</AlertDialogTitle>
            <AlertDialogDescription>
              আপনি কি নিশ্চিতভাবে সব অ্যাক্টিভিটি লগ মুছে ফেলতে চান? এই অ্যাকশনটি আর ফিরিয়ে আনা যাবে না।
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="rounded-xl">বাতিল</AlertDialogCancel>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90 rounded-xl" onClick={() => {
              ...
              setClearActivitiesConfirm(false);
            }}>
              সব মুছুন
            </AlertDialogAction>
          </AlertDialogFooter>
