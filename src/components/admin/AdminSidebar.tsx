...
const bottomItems = [
  ...
  { title: "স্টাফ ও রোল", url: "/admin/staff", icon: UserCheck },
  { title: "অপারেশনাল হেলথ", url: "/admin/operational-health", icon: Activity },
  { title: "মিডিয়া ও স্টোরেজ হাব", url: "/admin/media-library", icon: HardDrive },
  { title: "হোমপেজ", url: "/admin/homepage", icon: Home },
  { title: "SEO সেটিংস", url: "/admin/homepage-seo", icon: Globe },
...
export default function AdminSidebar() {
  const { signOut, user, role, isSuperAdmin, canAccess } = useAuth();
  ...
  const effectiveRole = isSuperAdmin ? "super_admin" : (role || "admin");
  const canSeeOrders = isSuperAdmin || canAccess("/admin/orders");
  const visibleOrderSubItems = isSuperAdmin ? orderSubItems : orderSubItems.filter(item => canAccess(item.url));
  const visibleBottomItems = isSuperAdmin ? bottomItems : bottomItems.filter(item => canAccess(item.url));
                    ...
                    <SidebarMenuItem>
                      ...
                      <CollapsibleContent>
                        <div className="ml-4 mt-1 space-y-0.5 border-l border-sidebar-border pl-3">
                          {visibleOrderSubItems.map((sub: any) => (
      ...
      <SidebarFooter className={`border-t border-sidebar-border ${collapsed ? "p-2" : "p-4"}`}>
        {!collapsed && (
          <NavLink to="/admin/profile" className="block w-full mb-2">
            <div className="rounded-xl bg-sidebar-accent/50 p-2.5 hover:bg-sidebar-accent transition-colors duration-200">
              ...
              <span className="inline-block mt-1 text-[9px] font-bold uppercase tracking-wider bg-accent/20 text-accent px-1.5 py-0.5 rounded">
                {(effectiveRole || "admin").replace("_", " ")}
              </span>
            </div>
          </NavLink>
        )}
